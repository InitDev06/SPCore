...
public class NickerCommand implements CommandExecutor {
	...
	public NickerCommand(SPCore spc) {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String la, String[] args) {
		...
		if(sender instanceof Player) {
			...
			if(player.hasPermission("spcore.cmd.nick") || player.isOp()) {
				if(args.length == 0) {
					player.sendMessage(Msg.color(lang.getString("messages.command.nick.usage")));
					return true;
				}
				
				String nickname = args[0];
				for(Player connected : Bukkit.getOnlinePlayers()) {
					if(connected.getDisplayName().equals(nickname)) {
						player.sendMessage(Msg.color(lang.getString("messages.command.nick.already_use")));
						return true;
					}
					...
					if(args[0].equalsIgnoreCase(nickname)) {
						...
						String message = lang.getString("messages.command.nick.changed");
					...
					if(args[0].equalsIgnoreCase(player.getName())) {
						...
						player.sendMessage(Msg.color(lang.getString("messages.command.nick.reset")));
						return true;
					}
				}
			} else {
				String message = lang.getString("messages.not_perm");
				message = message.replace("<permission>", "spcore.cmd.nick");
				if(config.getBoolean("options.sounds.reproduce_sounds")) {
					execute(player);
				}
				player.sendMessage(Msg.color(message));
